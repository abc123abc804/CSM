...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target):
        N, _, H, W = target.size()
        out = {}                                                                    #输出的out是一个字典，其中存储loss、bpp_loss、mse_loss、msssim_loss等
        num_pixels = N * H * W

        out["bpp_loss"] = sum(
            (torch.log(likelihoods).sum() / (-math.log(2) * num_pixels))
            for likelihoods in output["likelihoods"].values()                       #这里的R() = R(y) + R(z),因为这个概率包括了y和z的概率
        )
        if self.type == 'mse':
            out["mse_loss"] = self.mse(output["x_hat"], target)
            out["loss"] = self.lmbda * 255 ** 2 * out["mse_loss"] + out["bpp_loss"] #loss = R（） + lamdba * D（）
        else:
...
def train_one_epoch(
    ...
    model.train()
    device = next(model.parameters()).device                                #指定和参数相同的设备。

    for i, d in enumerate(train_dataloader):                                #i就是序号，d是dataloader中的每一张图像
        d = d.to(device)
        optimizer.zero_grad()                                               #梯度清零
        aux_optimizer.zero_grad()

        out_net = model(d)                                                  #model中forward输出的是一个字典

        out_criterion = criterion(out_net, d)                               #计算RD损失
        out_criterion["loss"].backward()                                    #反向传播  更新参数更新loss
        if clip_max_norm > 0:                                               #默认为1.0
            torch.nn.utils.clip_grad_norm_(model.parameters(), clip_max_norm)#梯度裁剪，防止梯度爆炸
        optimizer.step()
                                                                            #为什么只更新辅助loss？
        ...
        if i % 1000 == 0:                                                   #一个epoch中每1000张图像输出一下loss
            if type == 'mse':
                print(
                    f"Train epoch {epoch}: ["
...
def main(argv):
    ...
    train_transforms = transforms.Compose(                                      #将图像裁剪成256x256大小的图像，并转成张量
    ...
    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones, gamma=0.1, last_epoch=-1)          #学习率衰减策略
    ...
    for epoch in range(last_epoch, args.epochs):
        ...
        is_best = loss < best_loss                                                                          #这里的best_loss不是个字符串？？？？？？？？？？？？？？？？？
