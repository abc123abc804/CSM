...
#SwinT Block里边的一个小模块,真正的transformer
class WMSA(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, head_dim, window_size, type):
        ...
        self.n_heads = input_dim // head_dim  # in_dim = h * head_dim
        self.window_size = window_size
        self.type = type
        self.embedding_layer = nn.Linear(self.input_dim, 3 * self.input_dim, bias=True)  # 3 * h * self.head_dim
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b (w1 p1) (w2 p2) c -> b w1 w2 p1 p2 c', p1=self.window_size,p2=self.window_size)             # rearrange按给出的模式(注释)重组张量，其中模式中字母只是个表示，没有具体含义
        h_windows = x.size(1)
        w_windows = x.size(2)
        x = rearrange(x, 'b w1 w2 p1 p2 c -> b (w1 w2) (p1 p2) c', p1=self.window_size, p2=self.window_size)
        qkv = self.embedding_layer(x)
        # shape (h, b, nw, np, head_dim), (h, b, nw, np, head_dim), (h, b, nw, np, head_dim)
        q, k, v = rearrange(qkv, 'b nw np (threeh c) -> threeh b nw np c', c=self.head_dim).chunk(3, dim=0)  # 3 * h
...
# SwinT Block  根据type选择是W-MSA还是SW-MSA
...
#真正的TCM块   其实这一个ConvTransBlock只表示TCM块中的一个阶段，具体是那个阶段要根据type去判断
class ConvTransBlock(nn.Module):
    def __init__(self, conv_dim, trans_dim, head_dim, window_size, drop_path, type='W'):
        ...
        # self.conv_block = EASNDEEP()

    def forward(self, x):
        conv_x, trans_x = torch.split(self.conv1_1(x), (self.conv_dim, self.trans_dim), dim=1)                  #按照通道数进行拆分
        conv_x = self.conv_block(conv_x) + conv_x                                                        #前一半通道用于Conv，后一半通道用于SwinT
        trans_x = Rearrange('b c h w -> b h w c')(trans_x)
        trans_x = self.trans_block(trans_x)                                                             #trans_block就是SwinT block块
...
class ConvManbaVCBlock(nn.Module):
    def __init__(self, conv_dim, trans_dim, head_dim, window_size, drop_path, type='W'):
        """ SwinTransformer and Conv Block
        """
        super(ConvManbaVCBlock, self).__init__()
        self.conv_dim = conv_dim
        self.trans_dim = trans_dim
        self.head_dim = head_dim
        self.window_size = window_size
        ...
        # self.trans_block = Block(self.trans_dim, self.trans_dim, self.head_dim, self.window_size, self.drop_path,
        #                          self.type)
        # self.mambaVC_block = [VSSBlock(hidden_dim=256, drop_path=0.1, use_checkpoint=False,
        ...
        #                 for i in range(1)] + [conv(256, 256, kernel_size=3, stride=2)]
        self.mambaVC_block = VSSBlock(hidden_dim=128, drop_path=0.1, use_checkpoint=False,
                                       norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
        ...
        self.score = ChannelAttentionBlock()
    ...
    def forward(self, x):
        ...
        # trans_x = Rearrange('b c h w -> b h w c')(trans_x)
        # trans_x = self.trans_block(trans_x)                                                             #trans_block就是SwinT block块
        trans_x = self.mambaVC_block(trans_x)
        # trans_x = Rearrange('b h w c -> b c h w')(trans_x)
        res = self.conv1_2(torch.cat((conv_x, trans_x), dim=1))
        x = x + res
        return x

#这个模块就是non-local中提出来的non-local attention模块（三路分支）
class SWAtten(AttentionBlock):
    ...
    def forward(self, x):
        x = self.in_conv(x)
        identity = x
        z = self.non_local_block(x)                     #权重分支中的第一个处理  non-local块
        a = self.conv_a(x)                              #三个残差块堆叠
        b = self.conv_b(z)                              #三个残差快 + 1x1卷积
        out = a * torch.sigmoid(b)
        out += identity
        out = self.out_conv(out)
        return out

#这是SWAtten（三路分支）中的权重分支上的第一个处理，SwinT Block
...
class SwinCNN_1(nn.Module):

    def __init__(self, conv_dim, trans_dim, head_dim, window_size, drop_path, h, w, type='W'):
        super(SwinCNN_1, self).__init__()
        self.conv_dim = conv_dim
        self.trans_dim = trans_dim
        self.head_dim = head_dim
        self.window_size = window_size
    ...
    def forward(self, x):
        short_cut = self.identity(x)
        x = self.conv1_1(x)
        # x = torch.
        # print(x.shape)                                            #x:(8,256,64,64)
        b, c, h, w = x.shape

        # x = x.reshape(b, c, -1)
        # x1 = self.score(x)  # (b,c,1)
        # x = x * torch.softmax(x1, dim=1) + x
        # idx = torch.argsort(x1, 1, descending=True)
        # id = idx[:, :128, :]
        # id_r = idx[:, 128:, :]
        # conv_x = torch.gather(x, 1, id.tile(1, 1, x.shape[-1])).reshape(b, c // 2, h, w)
        # trans_x = torch.gather(x, 1, id_r.tile(1, 1, x.shape[-1])).reshape(b, c // 2, h, w)
        ...
        trans_x = Rearrange('b h w c -> b c h w')(trans_x)
        res = self.conv1_2(torch.cat((conv_x, trans_x), dim=1))
        short_cut = short_cut + res
        return short_cut

class SwinCNN_2(nn.Module):

    def __init__(self,conv_dim, trans_dim, head_dim, window_size, drop_path, type='SW'):
        super(SwinCNN_2, self).__init__()
        self.conv_dim = conv_dim
        self.trans_dim = trans_dim
        self.head_dim = head_dim
        self.window_size = window_size
...
class mambaVC1(nn.Module):

    def __init__(self, conv_dim, trans_dim, head_dim, window_size, drop_path, h, w, type='W'):
        super(mambaVC1, self).__init__()
        self.conv_dim = conv_dim
        self.trans_dim = trans_dim
        self.head_dim = head_dim
        self.window_size = window_size
...
class mambaVC2(nn.Module):

    def __init__(self,conv_dim, trans_dim, head_dim, window_size, drop_path, type='SW'):
        super(mambaVC2, self).__init__()
        self.conv_dim = conv_dim
        self.trans_dim = trans_dim
        self.head_dim = head_dim
        self.window_size = window_size
...
class ChannelAttentionBlock(nn.Module):
    def __init__(self, in_nc=1) -> None:
...
class EASN(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv1x1 = nn.Conv2d(128, 128, 1, 1, 0)
...
class EASNDEEP(nn.Module):
    def __init__(self):
        super(EASNDEEP, self).__init__()
        self.conv1 = nn.Conv2d(128, 128, 3, 1, 1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(128, 128, 3, 1, 1)
        self.sigmoid = nn.Sigmoid()
        self.conv1x1 = nn.Conv2d(128, 128, 1, 1, 0)
        self.conv5x5 = nn.Conv2d(128, 128, 5, 1, 2)

        self.conv3 = nn.Conv2d(128, 128, 3, 1, 1)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv4 = nn.Conv2d(128, 128, 3, 1, 1)
        self.sigmoid2 = nn.Sigmoid()
        self.conv5x5_2 = nn.Conv2d(128, 128, 5, 1, 2)
    ...
    def forward(self, x):
        ...
        x_3x3 = self.conv2(x_3x3)
        x_3x3 = self.sigmoid(x_3x3)
        x = x_3x3 * x_5x5
        x = x + x_1x1

        identity2 = x
        x_5x5_2 = self.conv5x5_2(x)
        x = self.conv3(x)
        x = self.relu2(x)
        x = self.conv4(x)
        x = self.sigmoid2(x)
...
def cross_selective_scan(
    ...
    x_proj_weight = x_proj_weight.to(x.device)
...
class TCM(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128, M=320,
        ...
        # self.m_down1 = [ConvManbaVCBlock(dim, dim, self.head_dim[0], self.window_size, dpr[i + begin],
        #                                'W' if not i % 2 else 'SW')
        #                 for i in range(config[0])] + \
        #                [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]

        self.m_down1 = [SwinCNN_1(dim, dim, self.head_dim[1], self.window_size, dpr[0 + begin], 64, 64,'W')] + \
                        [SwinCNN_2(dim, dim, self.head_dim[1], self.window_size, dpr[1 + begin], 'SW')]     + \
                       [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]
        begin += config[0]

        # self.m_down1 = [VSSBlock(hidden_dim=256, drop_path=0.1, use_checkpoint=False,
        #                          norm_layer=nn.LayerNorm, ssm_d_state=16, ssm_ratio=2.0, ssm_dt_rank="auto",
        #                          ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0, ssm_init="v0", forward_type="v2")
        #                 for i in range(2)] + [conv(256, 256, kernel_size=3, stride=2)]


        self.m_down2 = [ConvManbaVCBlock(dim, dim, self.head_dim[1], self.window_size, dpr[i + begin],
                                       'W' if not i % 2 else 'SW')
                        for i in range(config[1])] + \
                       [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]

        # self.m_down2 = [SwinCNN_1(dim, dim, self.head_dim[1], self.window_size, dpr[0 + begin], 64, 64,'W')] + \
        #                 [SwinCNN_2(dim, dim, self.head_dim[1], self.window_size, dpr[1 + begin], 'SW')]     + \
        #                [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]

        begin += config[1]

        self.m_down3 = [ConvManbaVCBlock(dim, dim, self.head_dim[2], self.window_size, dpr[i + begin],
                                       'W' if not i % 2 else 'SW')
                        for i in range(config[2])] + \
                       [conv3x3(2 * N, M, stride=2)]
        # self.m_down3 = [SwinCNN_1(dim, dim, self.head_dim[1], self.window_size, dpr[0 + begin], 64, 64,'W')] + \
        #                 [SwinCNN_2(dim, dim, self.head_dim[1], self.window_size, dpr[1 + begin], 'SW')]     + \
        #                [conv3x3(2 * N, M, stride=2)]

        begin += config[2]
        self.m_up1 = [ConvManbaVCBlock(dim, dim, self.head_dim[3], self.window_size, dpr[i + begin],
                                     'W' if not i % 2 else 'SW')
                      for i in range(config[3])] + \
                     [ResidualBlockUpsample(2 * N, 2 * N, 2)]
        # self.m_up1 = [SwinCNN_1(dim, dim, self.head_dim[4], self.window_size, dpr[0 + begin], 64, 64, 'W')] + \
        #              [SwinCNN_2(dim, dim, self.head_dim[4], self.window_size, dpr[1 + begin], 'SW')] + \
        #              [ResidualBlockUpsample(2 * N, 2 * N, 2)]

        self.m_up2 = [ConvManbaVCBlock(dim, dim, self.head_dim[4], self.window_size, dpr[i + begin],
                                     'W' if not i % 2 else 'SW')
                      for i in range(config[4])] + \
                     [ResidualBlockUpsample(2 * N, 2 * N, 2)]

        # self.m_up2 = [SwinCNN_1(dim, dim, self.head_dim[4], self.window_size, dpr[0 + begin], 64, 64, 'W')] + \
        #              [SwinCNN_2(dim, dim, self.head_dim[4], self.window_size, dpr[1 + begin], 'SW')] + \
        #              [ResidualBlockUpsample(2 * N, 2 * N, 2)]

        # self.m_up3 = [ConvManbaVCBlock(dim, dim, self.head_dim[5], self.window_size, dpr[i + begin],
        #                              'W' if not i % 2 else 'SW')
        #               for i in range(config[5])] + \
        #              [subpel_conv3x3(2 * N, 3, 2)]
        self.m_up3 = [SwinCNN_1(dim, dim, self.head_dim[4], self.window_size, dpr[0 + begin], 64, 64, 'W')] + \
                     [SwinCNN_2(dim, dim, self.head_dim[4], self.window_size, dpr[1 + begin], 'SW')] + \
                     [subpel_conv3x3(2 * N, 3, 2)]
    ...
    def forward(self, x):
        ...
        z_tmp = z - z_offset                                        #这里这两个操作就能够代表熵编解码器吗？
        z_hat = ste_round(z_tmp) + z_offset                         #量化操作，所以这是先量化，然后将z尖进行熵编码。因为熵编码时无损的，因此编解码完之后的z尖和这个z尖一摸一样。

        latent_scales = self.h_scale_s(z_hat)
        latent_means = self.h_mean_s(z_hat)

        y_slices = y.chunk(self.num_slices, 1)                      #1表示在第1维进行拆分，也就是在通道维进行拆分。
        y_hat_slices = []
        y_likelihood = []
        mu_list = []
        scale_list = []                                             #？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
        for slice_index, y_slice in enumerate(y_slices):            #这里最初始的时候，明明y_hat_slices是空的，也就是说support_slices中latent_means是与空的拼起来，不就相当于latent_means自己吗？
            support_slices = (y_hat_slices if self.max_support_slices < 0 else y_hat_slices[:self.max_support_slices])
            mean_support = torch.cat([latent_means] + support_slices, dim=1)                    #对应解码器中将Fmean与y拼起来
            mean_support = self.atten_mean[slice_index](mean_support)                           #送入SWAtten块中，然后通过slice_index找到各自的SWAtten模块
            ...
            y_hat_slice = ste_round(y_slice - mu) + mu             #y_hat_slice为什么是经过y_slice做一个修正得到的 ？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
            # if self.training:
            #     lrp_support = torch.cat([mean_support + torch.randn(mean_support.size()).cuda().mul(scale_support), y_hat_slice], dim=1)
            # else:
            lrp_support = torch.cat([mean_support, y_hat_slice], dim=1)                     #将Fmean和y_hat_slice拼起來
